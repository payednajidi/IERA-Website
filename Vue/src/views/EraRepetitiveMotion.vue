...
const rowTemplate = [
  ...
  },
  {
    ...
    body_part: 'Neck, shoulders, elbows, wrists, hands, knee',
    physical_risk_factor: 'Work involving intensive use of the fingers, hands or wrist or work involving intensive data entry (key-in)',
    max_exposure_duration: 'More than 3 hours on a normal workday',
  },
  {
...
const loadPage = async () => {
  try {
    ...
    const response = await api.get(`/era-repetitive-motion/${assessmentId}`)
...
const saveAndNext = async () => {
  ...
  try {
    await api.post('/era-repetitive-motion', {
      assessment_id: assessmentId,
      task_not_applicable: taskNotApplicable.value,
      rows: rows.value,
    })

    markStepCompleted(assessmentId, 4)
    router.push(`/era-vibration/${assessmentId}`)
  } catch (error) {
    console.error(error.response?.data || error)
    alert('Error saving repetitive motion checklist.')
  } finally {
    saving.value = false
  }
}
...
<template>
  <div v-if="loading" class="loading-state">Loading repetitive motion checklist...</div>

  <div v-else class="page-wrapper">
    <div class="page-hero">
      <div class="hero-left">
        <div class="hero-tag">Step 4 of 7</div>
        <h1 class="hero-title">Repetitive Motion</h1>
        <p class="hero-sub">File: `EraRepetitiveMotion.vue` - Repetitive motion risk factor checklist. Assessment ID: #{{ assessmentId }}</p>
      </div>
      <div class="hero-steps">
        <div v-for="s in 7" :key="s" class="step-pip" :class="{ active: s === 4 }">
        ...
        <table class="table">
          <thead>
            <tr>
              ...
              <th rowspan="2">Max. Exposure Duration</th>
          ...
          <tbody>
            ...
            <tr class="subtotal-row">
              <td colspan="3" class="subtotal-label">Sub Total (Number of tick(s))</td>
    ...
    <div class="action-row">
      <button class="btn-nav btn-prev" @click="router.push(`/era-forceful-exertion/${assessmentId}`)">Previous</button>
      <button class="btn-nav btn-next" :disabled="saving" @click="saveAndNext">
        {{ saving ? 'Saving...' : 'Save & Proceed (Step 5)' }}
      </button>
    </div>
  </div>
</template>
