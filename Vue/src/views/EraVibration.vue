...
const loadPage = async () => {
  try {
    ...
    const response = await api.get(`/era-vibration/${assessmentId}`)
...
const saveAndNext = async () => {
  ...
  try {
    await api.post('/era-vibration', {
      assessment_id: assessmentId,
      task_not_applicable: taskNotApplicable.value,
      rows: rows.value,
    })

    markStepCompleted(assessmentId, 5)
    router.push(`/era-environmental-factors/${assessmentId}`)
  } catch (error) {
    console.error(error.response?.data || error)
    alert('Error saving vibration checklist.')
  } finally {
    saving.value = false
  }
}
...
<template>
  <div v-if="loading" class="loading-state">Loading vibration checklist...</div>

  <div v-else class="page-wrapper">
    <div class="page-hero">
      <div class="hero-left">
        <div class="hero-tag">Step 5 of 7</div>
        <h1 class="hero-title">Vibration</h1>
        <p class="hero-sub">File: `EraVibration.vue` - Vibration risk factor checklist. Assessment ID: #{{ assessmentId }}</p>
      </div>
      <div class="hero-steps">
        <div v-for="s in 7" :key="s" class="step-pip" :class="{ active: s === 5 }">
    ...
    <div class="action-row">
      <button class="btn-nav btn-prev" @click="router.push(`/era-repetitive-motion/${assessmentId}`)">Previous</button>
      <button class="btn-nav btn-next" :disabled="saving" @click="saveAndNext">
        {{ saving ? 'Saving...' : 'Save & Proceed (Step 6)' }}
      </button>
    </div>
  </div>
</template>
